...
From this branch's checkout in Ubuntu/WSL, after [building and preparing local
assets](building.md):
...
bash tools/run-studio.sh --map MAP_ROUTE104 \
  ...
  --out build/my-bridge.json --connected
```
...
```bash
bash tools/run-studio.sh --map MAP_ROUTE104 --out build/my-bridge.json --connected
```
